...
//        });
    ...
    private void browseButton1ActionPerformed(java.awt.event.ActionEvent evt) 
    {
    	...
    	{
    		//Displays all paths by iterating all files, assuming they al belong on same directory
    		problemField.setText(problemChooser.getSelectedFiles()[0].toString());
    		for(int i=1; i<problemChooser.getSelectedFiles().length; i++)
    		{
    			String[] path = problemChooser.getSelectedFiles()[i].toString().split("/");
