...
public class textToXML {
	public static void convert(String inputName, String outputName) throws IOException
	{
		...
		output.write("\t\t\t<vehicle_count>" + input.nextInt() + "</vehicle_count>\n");
		...
		while(input.hasNextInt())
		{
			...
			output.write("\t\t<service_time>"+ input.nextInt() +"</service_time>\n");
			...
			for(int[] i : coords)
			{
				double distance = Math.sqrt(Math.pow(i[0]- coords.get(actual)[0], 2) + Math.pow(i[1]- coords.get(actual)[1], 2));
				distances[curr] = distance;
				
				output.write("\t\t\t<client_" + curr + ">"+ distance +"</client_"+curr+">\n");
				++curr;
			}
			output.write("\t\t</distances>\n");
			...
			for(int i=0; i<distances.length; i++)
				output.write("\t\t\t<client_"+i+">"+distances[i]+"</client_"+i+">\n");
			output.write("\t\t</times>\n");
